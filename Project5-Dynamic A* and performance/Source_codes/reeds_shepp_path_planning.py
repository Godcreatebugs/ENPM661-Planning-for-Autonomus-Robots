...
def interpolation(ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, directions):
...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    for (m, l, i) in zip(mode, lengths, range(len(mode))):
        ...
        while abs(pd) <= abs(l):
            ind += 1
            px, py, pyaw, directions = interpolation(
...
def path_calculation(sx, sy, syaw, gx, gy, gyaw, maxc, step_size):
    q0 = [sx, sy, syaw]
    q1 = [gx, gy, gyaw]

    paths = path_finding(q0, q1, maxc)
    for path in paths:
...
def rs_pathplanning(sx, sy, syaw,
                              gx, gy, gyaw, maxc, step_size):

    paths = path_calculation(sx, sy, syaw, gx, gy, gyaw, maxc, step_size)
...
def test():
    ...
    for i in range(NTEST):
        ...
        px, py, pyaw, mode, clen = rs_pathplanning(
...
def main():
    print("Reeds Shepp  planner starts")
